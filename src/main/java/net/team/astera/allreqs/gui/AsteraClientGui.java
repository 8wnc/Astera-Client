package net.team.astera.allreqs.gui;

import net.minecraft.client.MinecraftClient;
import net.team.astera.AsteraClient;
import net.team.astera.allreqs.Feature;
import net.team.astera.allreqs.gui.items.Item;
import net.team.astera.allreqs.gui.items.buttons.ModuleButton;
...
public class AsteraClientGui extends Screen {
    private static AsteraClientGui AsteraClientGui;
    private static AsteraClientGui INSTANCE;

    static {
        INSTANCE = new AsteraClientGui();
    }

    private final ArrayList<Component> components = new ArrayList();

    public AsteraClientGui() {
        super(Text.literal("AsteraClient"));
        setInstance();
        load();
    }

    public static AsteraClientGui getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new AsteraClientGui();
        }
        return INSTANCE;
    ...
    public static AsteraClientGui getClickGui() {
        return AsteraClientGui.getInstance();
    }

    public static boolean isCurrentGuiOpen() {
        return MinecraftClient.getInstance().currentScreen instanceof AsteraClientGui;
    }
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : AsteraClient.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {
                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    AsteraClient.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
