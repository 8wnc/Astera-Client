...
import net.team.astera.allreqs.gui.Component;
...
public class Slider
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.rect(context.getMatrices(), this.x, this.y, (this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? AsteraClient.colorManager.getColorWithAlpha(AsteraClient.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : AsteraClient.colorManager.getColorWithAlpha(AsteraClient.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        drawString(this.getName() + " " + Formatting.GRAY + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf((this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) AsteraClientGui.getClickGui().getTextOffset(), -1);
    }
    ...
    public boolean isHovering(int mouseX, int mouseY) {
        for (Component component : AsteraClientGui.getClickGui().getComponents()) {
