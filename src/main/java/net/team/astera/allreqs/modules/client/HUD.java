...
import net.team.astera.impl.util.cameraclip.FrameRateCounter;
import net.team.astera.impl.util.cameraclip.MathUtility;
...
public class HUD extends Module {
    ...
    private final Setting<Boolean> extraTps = this.register(new Setting<>("CurrentTPS", true, v-> tps.getValue()));
    ...
    private final Setting<Greeter> greetermode = this.register(new Setting<Greeter>("Greeter", Greeter.NAME, v -> this.greeter.getValue()));
    private final Setting<String> customGreeter = this.register(new Setting<String>("Custom", "AsteraClient On Top", v -> this.greetermode.getValue() == Greeter.CUSTOM));
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        event.getContext().drawTextWithShadow( mc.textRenderer, AsteraClient.NAME + " " + AsteraClient.VERSION, 2,/* x */ 2/* y */, ColorUtil.toARGB(red.getValue(), green.getValue(), blue.getValue(), alpha.getValue()) );
        ...
        if (arrayList.getValue())
            if (renderingUp.getValue()) {
                for (Module module : AsteraClient.moduleManager.getEnabledModules().stream().filter(Module::isDrawn).sorted(Comparator.comparing(module -> mc.textRenderer.getWidth(module.getFullArrayString()) * -1)).collect(Collectors.toList())) {
                    if (!module.isDrawn()) {
                        continue;
                    }
        ...
        if (renderingUp.getValue()) {
            if (speed.getValue()) {
                String str = "Speed " + Formatting.WHITE + AsteraClient.speedManager.getSpeedKpH()  + " km/h";
                i += 10;
            ...
            if (tps.getValue()) {
                String str = "TPS " + Formatting.WHITE + AsteraClient.serverManager.getTPS() + (extraTps.getValue() ? " [" + AsteraClient.serverManager.getCurrentTps() + "]" : "");
                i += 10;
                event.getContext().drawTextWithShadow(mc.textRenderer, Text.of(str), (width - getStringWidth(str) - 2), (height - 2 - i), color);
            }
            String fpsText = "FPS " + Formatting.WHITE + FrameRateCounter.INSTANCE.getFps();
            String str1 = "Ping " + Formatting.WHITE +  AsteraClient.serverManager.getPing();
        ...
        if(direction.getValue()){
            switch (mc.player.getHorizontalFacing()){
                case EAST -> direction1 =  Formatting.GRAY + " [" + Formatting.WHITE + "+X" + Formatting.GRAY + "]";
                case WEST -> direction1 = Formatting.GRAY + " [" + Formatting.WHITE + "-X" + Formatting.GRAY + "]";
                case NORTH -> direction1 = Formatting.GRAY + " [" + Formatting.WHITE + "-Z" + Formatting.GRAY + "]";
                case SOUTH -> direction1 = Formatting.GRAY + " [" + Formatting.WHITE + "+Z" + Formatting.GRAY + "]";
            }
        }

        String coordinates = String.format("XYZ: %s, %s, %s [%s, %s] %s",
                String.format("\u00a7f%.2f", posX),
                String.format("\u00a7f%.2f", posY),
                String.format("\u00a7f%.2f", posZ),
                String.format("%.2f", hposX),
                String.format("%.2f", hposZ),
                direction1);
