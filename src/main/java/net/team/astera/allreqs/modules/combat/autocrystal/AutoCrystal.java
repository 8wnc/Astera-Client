package net.team.astera.allreqs.modules.combat.autocrystal;
...
import net.team.astera.allreqs.modules.combat.autocrystal.util.autoCrystal.DeadManager;
import net.team.astera.allreqs.modules.combat.autocrystal.util.math.ExplosionUtil;
import net.team.astera.allreqs.modules.combat.autocrystal.util.math.MathUtil;
import net.team.astera.allreqs.modules.combat.autocrystal.util.player.InteractionUtil;
import net.team.astera.allreqs.modules.combat.autocrystal.util.player.InventoryUtil;
import net.team.astera.allreqs.modules.combat.autocrystal.util.player.PlayerUtil;
import net.team.astera.allreqs.modules.combat.autocrystal.util.player.SearchInvResult;
import net.team.astera.allreqs.modules.combat.autocrystal.AutoCrystal.Recalc;
import net.team.astera.allreqs.modules.combat.autocrystal.util.Timer;
...
import net.team.astera.impl.event.impl.tntaura.EventPostSync;
   ...
   @Subscribe
    public void onSync(EventSync e) {
        ...
        switch (targetLogic.getValue()) {
            case HP -> target = AsteraClient.combatManager.getTargetByHealth(targetRange.getValue());
            case Distance -> target = AsteraClient.combatManager.getNearestTarget(targetRange.getValue());
            case FOV -> target = AsteraClient.combatManager.getTargetByFOV(targetRange.getValue());
        }
        ...
        if (mc.player.getOffHandStack().getItem() != Items.END_CRYSTAL && autoGapple.getValue()
            ...
            SearchInvResult result = InventoryUtil.findItemInHotBar(Items.ENCHANTED_GOLDEN_APPLE);
            result.switchTo();
        }
        ...
        // CA Speed counter
        if (invTimer++ >= 20) {
            crystalSpeed = MathUtil.clamp(prevCrystalsAmount - InventoryUtil.getItemCount(Items.END_CRYSTAL), 0, 255);
        ...
        if (e.getEntity() instanceof EndCrystalEntity crystal && !placedCrystals.isEmpty()) {
            ...
            for (BlockPos bp : cache.keySet())
                if (crystal.squaredDistanceTo(bp.toCenterPos()) < 0.3 && timing.getValue() == Timing.NORMAL && breakTimer.passedMs(breakDelay.getValue())) {
                    ...
                    AsteraClient.asyncManager.run(() -> handleSpawn(crystal));
                }
        }
    }
    ...
    public void onBlockDestruct(EventSetBlockState e) {
        ...
        if (target != null && target.squaredDistanceTo(e.getPos().toCenterPos()) <= 4 && e.getState().isAir() && !e.getPrevState().isAir() && blockRecalcTimer.every(200)) {
            ...
            AsteraClient.asyncManager.run(() -> calcPosition(recalculate.getValue() == Recalc.FAST ? 2f : placeRange.getValue(), recalculate.getValue() == Recalc.FAST ? e.getPos().toCenterPos() : mc.player.getPos()));
        }
    }
    ...
    public boolean shouldPause() {
        ...
        return isEnabled() && switchPause.getValue() && !AsteraClient.playerManager.switchTimer.passedMs(switchDelay.getValue()) && !silent && !silentWeakness;
    }
    ...
    public void attackCrystal(EndCrystalEntity crystal) {
        ...
        int prevSlot = -1;
        SearchInvResult antiWeaknessResult = InventoryUtil.getAntiWeaknessItem();
        SearchInvResult antiWeaknessResultInv = InventoryUtil.findInInventory(itemStack ->
        ...
        PlayerInteractEntityC2SPacket attackPacket = PlayerInteractEntityC2SPacket.attack(crystal, mc.player.isSneaking());
    ...
    public void placeCrystal(BlockHitResult bhr) {
        ...
        SearchInvResult crystalResult = InventoryUtil.findItemInHotBar(Items.END_CRYSTAL);
        SearchInvResult crystalResultInv = InventoryUtil.findItemInInventory(Items.END_CRYSTAL);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        mc.getNetworkHandler().sendPacket(
                new PlayerInteractBlockC2SPacket(
                        ...
                        bhr,
                        PlayerUtil.getWorldActionId(mc.world)
                )
        );
    ...
    private @NotNull List<CrystalData> getPossibleCrystals(PlayerEntity target) {
        ...
        for (Entity ent : entities) {
            ...
            if (PlayerUtil.squaredDistanceFromEyes(ent.getPos()) > explodeRange.getPow2Value())
                continue;

            if (!InteractionUtil.canSee(ent) && PlayerUtil.squaredDistanceFromEyes(ent.getPos()) > explodeWallRange.getPow2Value())
                continue;

            if (!ent.isAlive())
                continue;

            float damage = ExplosionUtil.getExplosionDamage2(ent.getPos(), target);
            float selfDamage = ExplosionUtil.getSelfExplosionDamage(ent.getPos(), selfPredictTicks.getValue());
            ...
            if (protectFriends.getValue()) {
                ...
                for (PlayerEntity pl : players) {
                    if (!AsteraClient.friendManager.isFriend(pl)) continue;
                    float fdamage = ExplosionUtil.getExplosionDamage2(ent.getPos(), pl);
    ...
    public @Nullable PlaceData getPlaceData(BlockPos bp, PlayerEntity target) {
        ...
        float damage = target == null ? 10f : ExplosionUtil.getExplosionDamage2(crystalVec, target);
        float selfDamage = ExplosionUtil.getSelfExplosionDamage(crystalVec, selfPredictTicks.getValue());
        boolean overrideDamage = shouldOverrideDamage(damage, selfDamage);
        ...
        if (protectFriends.getValue()) {
            ...
            for (PlayerEntity pl : players) {
                if (!AsteraClient.friendManager.isFriend(pl)) continue;
                float fdamage = ExplosionUtil.getExplosionDamage2(crystalVec, pl);
                if (fdamage > selfDamage) {
                    selfDamage = fdamage;
                }
            }
    ...
    private @Nullable BlockHitResult getDefaultInteract(Vec3d crystalVector, BlockPos bp) {
        ...
        if (PlayerUtil.squaredDistanceFromEyes(crystalVector) > placeRange.getPow2Value())
            return null;

        BlockHitResult wallCheck = mc.world.raycast(new RaycastContext(InteractionUtil.getEyesPos(mc.player), crystalVector, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player));

        if (wallCheck != null && wallCheck.getType() == HitResult.Type.BLOCK && wallCheck.getBlockPos() != bp)
            if (PlayerUtil.squaredDistanceFromEyes(crystalVector) > placeWallRange.getPow2Value())
                return null;
    ...
    public BlockHitResult getStrictInteract(@NotNull BlockPos bp) {
        ...
        if (mc.player.getEyePos().getY() > upPoint) {
            ...
            for (Direction dir : Direction.values()) {
                ...
                float distance = PlayerUtil.squaredDistanceFromEyes(directionVec);
        ...
        if (PlayerUtil.squaredDistanceFromEyes(bestVector) > placeRange.getPow2Value())
            return null;

        BlockHitResult wallCheck = mc.world.raycast(new RaycastContext(InteractionUtil.getEyesPos(mc.player), bestVector, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player));

        if (wallCheck != null && wallCheck.getType() == HitResult.Type.BLOCK && wallCheck.getBlockPos() != bp)
            if (PlayerUtil.squaredDistanceFromEyes(bestVector) > placeWallRange.getPow2Value())
                return null;
        ...
        for (float x = 0f; x <= 1f; x += 0.2f) {
            for (float y = 0f; y <= 1f; y += 0.2f) {
                for (float z = 0f; z <= 1f; z += 0.2f) {
                    ...
                    float distance = PlayerUtil.squaredDistanceFromEyes(point);

                    BlockHitResult wallCheck = mc.world.raycast(new RaycastContext(InteractionUtil.getEyesPos(mc.player), point, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player));
                    if (wallCheck != null && wallCheck.getType() == HitResult.Type.BLOCK && wallCheck.getBlockPos() != bp)
                        if (distance > placeWallRange.getPow2Value())
                            continue;


                    BlockHitResult result = ExplosionUtil.rayCastBlock(new RaycastContext(InteractionUtil.getEyesPos(mc.player), point, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player), bp);
