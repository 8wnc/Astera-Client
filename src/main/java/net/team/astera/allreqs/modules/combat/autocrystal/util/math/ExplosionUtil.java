package net.team.astera.allreqs.modules.combat.autocrystal.util.math;
...
import net.team.astera.impl.mixin.autototem.IExplosion;
...
public final class ExplosionUtil {
    ...
    public static float getExplosionDamage2(Vec3d crystalPos, PlayerEntity target) {
        try {
            ...
            return getExplosionDamageWPredict(crystalPos, target, PredictUtil.predictPlayer(target, AutoCrystal.getInstance().predictTicks.getValue()));
    ...
    /*public synchronized static float getAnchorExplosionDamage(BlockPos anchorPos, PlayerEntity target) {
        ...
        if (AutoAnchor.predictTicks.getValue() == 0)
            ...
            final_result = getExplosionDamageWPredict(anchorPos.toCenterPos(), target, PredictUtil.predictPlayer(target, AutoAnchor.predictTicks.getValue()));
    ...
    public static float getSelfExplosionDamage(Vec3d explosionPos, int predictTicks) {
        if (predictTicks == 0)
            ...
            return getExplosionDamageWPredict(explosionPos, mc.player, PredictUtil.predictPlayer(mc.player, predictTicks));
    }
