package net.team.astera.allreqs.modules.combat.autocrystal.util.player;
...
import static net.team.astera.allreqs.modules.Module.mc;
...
public final class InteractionUtil {
    ...
    public static boolean placeBlock(BlockPos bp, boolean rotate, Interact interact, PlaceMode mode, int slot, boolean returnSlot, boolean ignoreEntities) {
        ...
        if (slot != -1) InventoryUtil.switchTo(slot);
        else return false;

        boolean result = placeBlock(bp, rotate, interact, mode, ignoreEntities);

        if (returnSlot) InventoryUtil.switchTo(prevItem);
        return result;
    }
    ...
    public static BlockHitResult getPlaceResult(@NotNull BlockPos bp, Interact interact, boolean ignoreEntities) {
        ...
        if(interact == Interact.AirPlace) {
            return ExplosionUtil.rayCastBlock(new RaycastContext(InteractionUtil.getEyesPos(mc.player), bp.toCenterPos(), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player), bp);
        }
