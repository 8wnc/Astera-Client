package net.team.astera.allreqs.settings;
...
public class Setting<T> {
    ...
    private Setting<Boolean> parent;

    public Setting<T> withParent(Setting<Boolean> parent) {
        this.parent = parent;
        return this;
    }

    public boolean isParentEnabled() {
        return parent == null || parent.getValue();
    }
    ...
    public boolean isVisible() {
        ...
        return isParentEnabled();
         ...
         return isParentEnabled() && this.visibility.test(this.getValue());
    }
    
    public double getPow2Value() {
        if (this.value instanceof Number) {
        return Math.pow(((Number) this.value).doubleValue(), 2);
    }
        throw new UnsupportedOperationException("Pow2Value can only be calculated for numeric types.");
    }
}